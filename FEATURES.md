...
| 性能与缓存 | 有界脚本/表达式缓存、数字分类避免失败的大整数解析、标量列表/字典延迟序列化 | 全部对象表示、字节码、累计内存治理；0.11.0 七组通用负载仍慢约 3.8–7.7 倍，三组转换仍慢约 7.8–10.4 倍；部分负载较 0.10.1 回退约 11%–20% |
| 证据 | 855 值传递原生程序一致 + 4970 转换程序中 4969 一致/1 明确差异 + 1866 Unicode/字符串 + 65536 码位数据审计 + 1019 替换/完整性 + 321 异常 + 312 命名空间 + 340 缓存 + 726 语义 + 248 原有系统 Tcl 场景，两个编译后端，真实宿主验证 | 全量上游测试、跨操作系统、代表性性能和应用迁移 |
