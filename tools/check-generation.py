...
paths+=['evidence/'+suite+'-reference.json' for suite in ['semantic','cache','namespace','completion','syntax']]
def snapshot():
...
for cycle in range(2):
    ...
    for suite in ['semantic','cache','namespace','completion','syntax']:run(sys.executable,'tools/generate-semantic-oracle.py',suite)
...
(root/'evidence/generation-syntax.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8',newline='\n')
