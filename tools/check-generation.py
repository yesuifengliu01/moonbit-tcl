...
paths+=['evidence/'+suite+'-reference.json' for suite in ['semantic','cache','namespace','completion','syntax','unicode','conversion','value','switch','regexp','search','sort','array']]
paths += ['evidence/switch-reference-limits.json','evidence/regexp-reference-limits.json','evidence/regexp-start-reference.json','evidence/search-reference-limits.json','evidence/sort-reference-limits.json','evidence/array-reference-limits.json']
...
for cycle in range(2):
    run(sys.executable,'tools/generate-unicode.py')
    run(sys.executable,'tools/generate_list_oracle.py')
    run(sys.executable,'tools/probe-regexp-start.py')
    for suite in ['semantic','cache','namespace','completion','syntax','unicode','conversion','value','switch','regexp','search','sort','array']:run(sys.executable,'tools/generate-semantic-oracle.py',suite)
...
(root/'evidence/generation-array.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8',newline='\n')
