...
paths+=['evidence/'+suite+'-reference.json' for suite in ['semantic','cache','namespace','completion','syntax','unicode','conversion','value','switch','regexp','search']]
paths += ['evidence/switch-reference-limits.json','evidence/regexp-reference-limits.json','evidence/regexp-start-reference.json','evidence/search-reference-limits.json']
...
for cycle in range(2):
    ...
    for suite in ['semantic','cache','namespace','completion','syntax','unicode','conversion','value','switch','regexp','search']:run(sys.executable,'tools/generate-semantic-oracle.py',suite)
...
(root/'evidence/generation-search.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8',newline='\n')
