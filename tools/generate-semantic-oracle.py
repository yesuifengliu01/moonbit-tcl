...
if suite not in {'semantic','cache','namespace','completion','syntax','unicode','conversion','value','switch','regexp','search','sort'}:raise SystemExit('suite must be semantic, cache, namespace, completion, syntax, unicode, conversion, value, switch, regexp, search or sort')
...
if suite in {'switch','regexp','search','sort'}:
...
for case in ([] if suite in {'switch','regexp','search','sort'} else cases):
    t=tkinter.Tcl()
...
if suite in {'switch','regexp','search','sort'}:
    ...
    for start in range(0,len(rows),40):
        ...
        lines += ['}']
for row in ([] if suite in {'switch','regexp','search','sort'} else rows):
