...
if suite not in {'semantic','cache','namespace','completion','syntax','unicode','conversion','value','switch','regexp','search','sort','array'}:raise SystemExit('suite must be semantic, cache, namespace, completion, syntax, unicode, conversion, value, switch, regexp, search, sort or array')
...
if suite in {'switch','regexp','search','sort','array'}:
...
for case in ([] if suite in {'switch','regexp','search','sort','array'} else cases):
    t=tkinter.Tcl()
...
if suite in {'switch','regexp','search','sort','array'}:
    ...
    for start in range(0,len(rows),40):
        ...
        lines += ['}']
for row in ([] if suite in {'switch','regexp','search','sort','array'} else rows):
