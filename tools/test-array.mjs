...
const steps=JSON.parse(fs.readFileSync(new URL('./array-host-cases.json',import.meta.url),'utf8'));
...
try{
 ...
 same(JSON.parse(session_eval(b,'array exists inventory')).result,'0');
...
try{
 ...
 same((await send({source:'set touched kept;array anymore inventory missing'})).ok,false);
 same((await send({source:'list after failure $touched'})).result,'after failure kept');
 same((await send({reset:true})).ok,true);
 same((await send({source:'array exists inventory'})).result,'0');
}finally{clearTimeout(timeout);await worker.terminate()}
const report={checks,passed:checks,engineSHA256:createHash('sha256').update(fs.readFileSync(new URL('../web/engine.mjs',import.meta.url))).digest('hex'),scope:'Persistent exported sessions, real JSONL CLI, and actual web Worker through a Node adapter. Array cursor persistence/invalidation/reuse, empty keys, alias retention/deletion/procedure cleanup, hash expansion/statistics, prefixes, isolated UTF-16 keys, numeric values, failure recovery and reset. Browser acceptance is separate.'};
fs.writeFileSync(new URL('../evidence/array-hosts.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
