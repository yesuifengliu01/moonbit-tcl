...
const steps=JSON.parse(fs.readFileSync(new URL('./search-host-cases.json',import.meta.url),'utf8'));
...
try{
 ...
 same((await send({source:'set touched kept;lsearch -regexp {} {(}'})).ok,false);
...
const report={checks,passed:checks,engineSHA256:createHash('sha256').update(fs.readFileSync(new URL('../web/engine.mjs',import.meta.url))).digest('hex'),scope:'Persistent module sessions, real JSONL CLI and actual web Worker through a Node message adapter. Collection regexps, nested indices, binary boundaries, dictionary ordering, numeric representation, Unicode, failure recovery and reset. Browser checks are separate.'};
fs.writeFileSync(new URL('../evidence/search-hosts.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
