...
const steps=JSON.parse(fs.readFileSync(new URL('./sort-host-cases.json',import.meta.url),'utf8'));
...
try{
 ...
 same((await send({source:'set touched kept;lsort -stride 1 {}'})).ok,false);
...
const report={checks,passed:checks,engineSHA256:createHash('sha256').update(fs.readFileSync(new URL('../web/engine.mjs',import.meta.url))).digest('hex'),scope:'Persistent exported sessions, real JSONL CLI, and actual web Worker through a Node adapter. Group sorting and original indices, nested paths, stable uniqueness, numeric conversion, comparator order/case/completion/reentrancy, failure recovery and reset. Browser acceptance is separate.'};
fs.writeFileSync(new URL('../evidence/sort-hosts.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
