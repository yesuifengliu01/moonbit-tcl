...
const steps=[
 ['set x 0;set template {[incr x]:$x};subst $template','1:1'],
 ['subst $template','2:2'],
 ['list [subst {prefix[break]after[incr x]}] $x','prefix 2'],
 ['list [subst {before[continue]after[incr x]}] $x','beforeafter3 3'],
 ['array set a {index VALUE};subst -nocommands {[incr x] $a([set key index])}','[incr x] VALUE'],
 ['subst {before$a([return custom])after}','beforecustomafter'],
 ['info complete "foreach n {1 2} {\\n"','0'],
 ['list [info complete {[incr untouched]}] [info exists untouched] [subst $template]','1 0 4:4'],
];
...
try{
 ...
 same(JSON.parse(session_eval(b,'info exists template')).result,'0');
...
try{
 ...
 same((await send({source:'subst {[incr x][error bad]}'})).ok,false);
 same((await send({source:'list after failure $x'})).result,'after failure 5');
 same((await send({reset:true})).ok,true);
 same((await send({source:'info exists template'})).result,'0');
}finally{clearTimeout(timeout);await worker.terminate()}
const report={checks,passed:checks,engineSHA256:createHash('sha256').update(fs.readFileSync(new URL('../web/engine.mjs',import.meta.url))).digest('hex'),scope:'Persistent module sessions, real JSONL CLI and the actual web Worker module with a Node message adapter; substitution flags, exceptional completion, persistent side effects and no-execution completeness checks; separate browser evidence required.'};
fs.writeFileSync(new URL('../evidence/syntax-hosts.json',import.meta.url),JSON.stringify(report,null,2)+'\n');
